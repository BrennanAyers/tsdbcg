...
describe GameChannel do
  ...
  it "Renders Game State" do
    ...
    expect((transmissions.first)).to eq( response)
    expect(@player.cards.count).to eq(2)
    expect(@player.cards.first.card.name).to eq("Gold")
    expect(@player.cards.last.card.name).to eq("Estate")
  end
end
